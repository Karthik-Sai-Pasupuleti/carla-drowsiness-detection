...
class CameraBasedFeatureExtractor:
    ...
    def __init__(self, fps: float, window_duration_sec: float = 30.0):
        self.fps = fps
...
def camera_feature_extraction_main(shared_data, ready_event):
    ...
    window_duration_sec = 30  # desired metric interval
    last_metrics_time = time.time()
    last_metrics = None
    window_start_time = last_metrics_time
    ...
    while cap.isOpened():
        ...
        if (current_time - last_metrics_time) >= window_duration_sec:
            metrics = extractor.get_metrics()

            if metrics:
                # print("metrics:", metrics)
                # print(f"Duration since last metrics: {current_time - last_metrics_time:.2f} seconds")
                last_metrics = metrics
                last_metrics_time = current_time  # reset timer

                shared_data["blink_frequency"] = metrics["blink_frequency"]
                shared_data["perclos"] = metrics["perclos"]
                shared_data["yawn_frequency"] = metrics["yawn_frequency"]
                shared_data["blinks"] = metrics["blinks"]
                shared_data["camera_start"] = time.strftime("%H:%M:%S", time.localtime(window_start_time))
                shared_data["camera_end"] = time.strftime("%H:%M:%S", time.localtime(last_metrics_time))
...
if __name__ == "__main__":
    shared_data = dict()
    camera_feature_extraction_main(shared_data, ready_event)
