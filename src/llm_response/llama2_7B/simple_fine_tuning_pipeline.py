"""
LLaMA-2 Fine-Tuning Script with LoRA + Prefix Vector Logging to FAISS DB

This script:
1. Loads a quantized LLaMA-2 model with LoRA adapters.
2. Loads and tokenizes driver monitoring data.
3. Builds a PyTorch dataset and Trainer object.
4. Logs 9D feature vectors to a FAISS vector database.
5. Trains the model and logs losses to CSV.
6. Saves the fine-tuned model and tokenizer for inference.
...
class LossLoggerCallback(TrainerCallback):
    """
    Custom callback to log training loss into a CSV file during training.
    """

    def on_log(self, args, state, control, logs=None, **kwargs):
        """
        Called during logging. Appends the loss to 'loss_log.csv'.

        Args:
            args: TrainingArguments
            state: TrainerState
            control: TrainerControl
            logs (dict): Dictionary containing training metrics like 'loss'.
        """
...
def train():
    """
    Orchestrates the full fine-tuning workflow for LLaMA-2 with LoRA and prefix vector logging.
    """
    # === 1. Load Tokenizer ===
    ...
    training_args = TrainingArguments(
        ...
        bf16=True,
        remove_unused_columns=False  # <-- 🔥 CRITICAL FIX to preserve additional inputs
    )
